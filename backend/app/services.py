...
def update_search_index(skill: dict):
    ...
    def updater(data):
        ...
        # 构建完整搜索文本
        full_text = (
            f"{text} {author_name} {author_department} {author_organization} {tags}"
        )
...
def search_skills(
    ...
    if (
        not query.strip()
        and not filter_author
        and not filter_department
        and not filter_tag
    ):
        return []
    ...
    # 按下载量排序（使用 skills.json 中的值，因为搜索功能不直接展示统计）
...
def get_skill_versions(skill_id: str) -> List[dict]:
    ...
    versions = [v for v in data.get("versions", []) if v["skill_id"] == skill_id]
...
def delete_skill(skill_id: str) -> bool:
    ...
    try:
        ...
        def delete_versions(data):
            data["versions"] = [
                v for v in data.get("versions", []) if v["skill_id"] != skill_id
            ]
