...
def aggregate_daily(date: str = None) -> dict:
    ...
    for event in events:
        ...
        if etype == "skill.view":
            ...
            slug = meta.get("slug", "")
            # 同一个IP对同一个技能每天只算1次下载
            download_key = (slug, ip)
            if download_key not in download_tracker:
                download_tracker.add(download_key)
    ...
    result = {
        ...
        "searches": {
            ...
            "zero_result_rate": round(
                len(zero_result_searches) / len(searches) * 100, 1
            )
            if searches
            else 0,
            "top_queries": [
                {"query": q, "count": c} for q, c in search_queries.most_common(10)
            ],
        },
        "tags": {
...
def _accumulate_region(
    meta: dict, region_counter: Counter, center_counter: Counter, dc_counter: Counter
):
    """根据事件元数据累加区域统计"""
...
def _accumulate_region_by_author(
    author: str, region_counter: Counter, center_counter: Counter
):
    ...
    staff = next(
        (s for s in staff_data.get("staff", []) if s.get("name") == author), None
    )
...
def get_kpi_summary() -> dict:
    ...
    return {
        ...
        "yesterday": {
            "skills_total": _sum_key(
                [yesterday_metrics], ["skills", "total_publishes"]
            ),
        ...
        "this_week": {
            ...
            "unique_users": max(
                [_sum_key([m], ["summary", "unique_users"]) for m in week_metrics]
                or [0]
            ),
        },
        "this_month": {
            ...
            "unique_users": max(
                [_sum_key([m], ["summary", "unique_users"]) for m in month_metrics]
                or [0]
            ),
        },
    }
